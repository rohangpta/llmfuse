...
def test_arithmetic_coding():
    """Test the arithmetic coding implementation with a simple example."""
    ...
    try:
        ...
        ratio = estimate_compression_ratio(symbols, probabilities)
...
def test_long_sequence():
    """Test with a long sequence to verify no precision loss."""
    print("\nTesting Long Sequence (100 symbols)")
    print("=" * 50)
    
    coder = ArithmeticCoder()
    
    # Create a long sequence with varying probabilities
    symbols = [i % 10 for i in range(100)]  # 100 symbols from 0-9
    probabilities = []
    
    for i in range(100):
        # Create different probability distributions for each position
        probs = [0.1] * 10  # Base probability
        probs[i % 10] = 0.5  # Higher probability for the actual symbol
        total = sum(probs)
    ...
    try:
        ...
        print(f"Encoded {len(symbols)} symbols to {len(encoded)} bytes")
        print(f"Compression ratio: {len(symbols) * 2 / len(encoded):.2f}x")
        print(f"Perfect roundtrip: {symbols == decoded}")
        
        if symbols != decoded:
            # Find first difference
            for i, (orig, dec) in enumerate(zip(symbols, decoded)):
                if orig != dec:
                    print(f"First difference at position {i}: expected {orig}, got {dec}")
                    break
        
        return symbols == decoded
        
    except Exception as e:
        print(f"Error during long sequence test: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success1 = test_arithmetic_coding()
    success2 = test_long_sequence()
    
    if success1 and success2:
        print("\n🎉 All tests passed! Arithmetic coding is working correctly.")
    else:
        print("\n❌ Some tests failed.") 